...
class HAProxyMetrics < Sensu::Plugin::Metric::CLI::Graphite
  ...
  CSV_FIELDS = {
    ...
    83 => %w[wrew wrew]
    84 => %w[connect connect]
    85 => %w[reuse reuse]
    86 => %w[cache_lookups cache_lookups]
    87 => %w[cache_hits cache_hits]
    88 => %w[srv_icur srv_icur]
    89 => %w[src_ilim src_ilim]
    90 => %w[qtime_max qtime_max]
    91 => %w[ctime_max ctime_max]
    92 => %w[rtime_max rtime_max]
    93 => %w[ttime_max ttime_max]
    94 => %w[eint eint]
    95 => %w[idle_conn_cur idle_conn_cur]
    96 => %w[safe_conn_cur safe_conn_cur]
    97 => %w[used_conn_cur used_conn_cur]
    98 => %w[need_conn_est need_conn_est]
    99 => %w[uweight uweight]
    100 => %w[agg_server_check_status agg_server_check_status]
    101 => %w[ssl_sess ssl_sess]
    102 => %w[ssl_reused_sess ssl_reused_sess]
    103 => %w[ssl_failed_handshake ssl_failed_handshake]
    104 => %w[h2_headers_rcvd h2_headers_rcvd]
    105 => %w[h2_data_rcvd h2_data_rcvd]
    106 => %w[h2_settings_rcvd h2_settings_rcvd]
    107 => %w[h2_rst_stream_rcvd h2_rst_stream_rcvd]
    108 => %w[h2_goaway_rcvd h2_goaway_rcvd]
    109 => %w[h2_detected_conn_protocol_errors h2_detected_conn_protocol_errors]
    110 => %w[h2_detected_strm_protocol_errors h2_detected_strm_protocol_errors]
    111 => %w[h2_rst_stream_resp h2_rst_stream_resp]
    112 => %w[h2_goaway_resp h2_goaway_resp]
    113 => %w[h2_open_connections h2_open_connections]
    114 => %w[h2_backend_open_streams h2_backend_open_streams]
    115 => %w[h2_total_connections h2_total_connections]
    116 => %w[h2_backend_total_streams h2_backend_total_streams]
    117 => %w[h1_open_connections h1_open_connections]
    118 => %w[h1_open_streams h1_open_streams]
    119 => %w[h1_total_connections h1_total_connections]
    120 => %w[h1_total_streams h1_total_streams]
    121 => %w[h1_bytes_in h1_bytes_in]
    122 => %w[h1_bytes_out h1_bytes_out]
    123 => %w[h1_spliced_bytes_in h1_spliced_bytes_in]
    124 => %w[h1_spliced_bytes_out h1_spliced_bytes_out]
  }.freeze
